...
def add_noise(x, noise_factor=0.05):
    ...
    return np.clip(noisy_x, -1.0, 1.0)  # Clip values to stay within the valid range

# Training the model with data augmentation (adding noise)
def train_model(model, epochs, optimizer, noise_factor=3.0):
    batch_size = 256
    model.compile(optimizer=optimizer, loss='sparse_categorical_crossentropy', metrics=['accuracy'])
    
    # Data augmentation: Adding noise to input samples during training
    noisy_X_train = add_noise(X_train, noise_factor=noise_factor)
...
model.save('./model/audioNoise.keras')
model.save('./model/audioNoise.h5')
